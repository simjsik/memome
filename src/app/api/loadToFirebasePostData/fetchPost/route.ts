...
export async function POST(req: NextRequest) {
    ...
    console.log('데이터 확인', userId, startAfterParam, pageSize)
    console.log('Received pageParam:', pageParam);
    console.log('Transformed startAfterParam:', startAfterParam);
    if (startAfterParam) {
        console.log('startAfterParam:', startAfterParam);
        console.log('Timestamp instance:', startAfterParam[1] instanceof Timestamp);
    }
    try {
        // 닉네임 매핑을 위한 캐시 초기화
