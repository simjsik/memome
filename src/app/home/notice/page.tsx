...
export default function ClientNotice() {
    ...
    const {
        ...
        getNextPageParam: (lastPage) => {
            if (!lastPage.nextPage) return undefined; // 더 이상 데이터가 없으면 undefined 반환

            return lastPage.nextPage as [boolean, Timestamp] | [boolean, null]; // 정확한 타입 설정
        },
        staleTime: 5 * 60 * 1000,
        initialPageParam: [true, null] as [boolean, Timestamp] | [boolean, null],
        initialData: {
            pages: [
                {
                    data: [] as PostData[],
                    nextPage: [true, null] as [boolean, null],
                },
            ],
            pageParams: [[true, null] as [boolean, null]],
        },
    });
